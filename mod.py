...
class ModVote:
	...
	def step( self, event ):
		if self.activeVote is not None:
			av = self.activeVote
			# if vote timed out
		...
		if event is not None:
			if event.type == "private":
				...
				if m[0] == "help":
					self.bot.send("vote create <question>: create a new, empty vote. You can have only one vote at a time",
								  event.nick)
					...
					self.bot.send(
							"vote send <timeout> : Send your vote in the channel (where the bot is) for 60 sec, timeout sec if precised (min: 30, max: 300)",
							event.nick)
				...
				if m[0] == "vote" and len(m)>1:
					...
					# Set Question
						...
						if len(m)>1:
							...
							self.bot.send("No question set. Use like this : \"set_question Does pinguins have knees ?\"",
										  event.nick)
							return
					# Add Choice
					elif m[0] == "add":
					...
					# If no command (a client is voting ?)
						...
						if self.activeVote is not None:
							...
							try:
								...
								self.bot.send(
										"Maybe you tried to type a command and failed, please type \"/msg {0} vote help\" for help".format(
												self.bot.nick), event.nick)
							else:
								if 1<=v<=len(self.activeVote.choice):
